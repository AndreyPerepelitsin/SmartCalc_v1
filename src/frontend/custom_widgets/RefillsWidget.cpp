...
RefillsWidget::RefillsWidget(QWidget *parent) : QWidget(parent) {
  mainLayout = new QVBoxLayout(this);

  buttonLayout = new QHBoxLayout();

  addButton = new QPushButton("Add refill", this);
  removeButton = new QPushButton("Remove refill", this);

  QPushButton *buttons[] = {addButton, removeButton};

  for (int i = 0; i < 2; i++) {
    buttons[i]->setSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed);
    buttons[i]->setFixedSize(80, 14);
    setStyleDefaultPushButtonWidget(buttons[i]);
  }

  connect(addButton, &QPushButton::clicked, this, &RefillsWidget::addLayout);
...
QVBoxLayout *RefillsWidget::getLayout() const { return mainLayout; }

int RefillsWidget::setRefills(DepositParameters *depositParams) const {
  int error_code = ErrorCodeFrontDeposit_noError;
  int return_error_code = ErrorCodeFrontDeposit_noError;

  for (const QHBoxLayout *layout : layouts) {
    error_code = ErrorCodeFrontDeposit_noError;
    ...
    if (!transactionPeriodicityElement || !transactionDateElement ||
      ...
      error_code = ErrorCodeFrontDeposit_unableToFindChildren;
    }
    if (!error_code) {
      ...
      if (!isDoubleNumber(transactionAmount)) {
        error_code = ErrorCodeFrontDeposit_incorrectTransactionValue;
      ...
      if (!error_code) {
        ...
        if (transactionAmountValue < S21_MIN_TRANSACTION_AMOUNT_VALUE ||
          ...
          addRefillTransaction(depositParams, transactionAmountValue, rawDate,
                               interval);
        }
      }
    }
...
void RefillsWidget::removeLayout() {
  if (!layouts.isEmpty()) {
