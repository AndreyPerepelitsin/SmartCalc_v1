...
void MainWindow::setupConnections() {
  ...
  connect(ui->pushButton_clear, SIGNAL(clicked()), this,
          SLOT(handleClearButtonClick()));
  connect(ui->pushButton_equals, SIGNAL(clicked()), this,
          SLOT(handleButtonEqualsClick()));

  connect(ui->lineEdit, SIGNAL(returnPressed()), this,
          SLOT(handleButtonEqualsClick()));
  connect(ui->lineEdit, SIGNAL(textChanged(const QString &)), this,
          SLOT(handleTextChanged_lineEdit(const QString &)));

  connect(ui->lineEdit_X, SIGNAL(returnPressed()), this,
          SLOT(handleButtonEqualsClick()));
  connect(ui->lineEdit_X, SIGNAL(textChanged(const QString &)), this,
          SLOT(handleTextChanged_variable(const QString &)));
}

void MainWindow::setupGraphCalculatorElements() {
  connect(ui->lineEdit_domain, SIGNAL(returnPressed()), this,
          SLOT(handleDrawPlot()));
  connect(ui->lineEdit_domain, SIGNAL(textChanged(const QString &)), this,
          SLOT(handleTextChanged_domainEdit(const QString &)));

  connect(ui->lineEdit_codomain, SIGNAL(returnPressed()), this,
          SLOT(handleDrawPlot()));
  connect(ui->lineEdit_codomain, SIGNAL(textChanged(const QString &)), this,
          SLOT(handleTextChanged_codomainEdit(const QString &)));

  connect(ui->pushButton_graph_draw, SIGNAL(clicked()), this,
          SLOT(handleDrawPlot()));

  connect(ui->lineEdit_graph_expression, SIGNAL(returnPressed()), this,
          SLOT(handleDrawPlot()));
  connect(ui->lineEdit_graph_expression, SIGNAL(textChanged(const QString &)),
          this, SLOT(handleTextChanged_graphExpression(const QString &)));
...
void MainWindow::connectCreditCalculatorElements() {
  connect(ui->pushButton_credit_calculate, SIGNAL(clicked()), this,
          SLOT(handleButtonCreditCalculate()));

  connect(ui->lineEdit_credit_creditAmount, SIGNAL(returnPressed()), this,
          SLOT(handleButtonCreditCalculate()));
  connect(ui->lineEdit_credit_creditAmount,
          SIGNAL(textChanged(const QString &)), this,
          SLOT(handleTextChanged_creditAmount(const QString &)));

  connect(ui->lineEdit_credit_creditDeadline, SIGNAL(returnPressed()), this,
          SLOT(handleButtonCreditCalculate()));
  connect(ui->lineEdit_credit_creditDeadline,
          SIGNAL(textChanged(const QString &)), this,
          SLOT(handleTextChanged_creditDeadline(const QString &)));

  connect(ui->lineEdit_credit_interestValue, SIGNAL(returnPressed()), this,
          SLOT(handleButtonCreditCalculate()));
  connect(ui->lineEdit_credit_interestValue,
          SIGNAL(textChanged(const QString &)), this,
          SLOT(handleTextChanged_interestValue(const QString &)));
...
void MainWindow::connectDepositCalculatorElements() {
  connect(ui->pushButton_deposit_calculate, SIGNAL(clicked()), this,
          SLOT(handleDepositCalculateButtonClick()));

  connect(ui->lineEdit_deposit_depositAmount,
          SIGNAL(textChanged(const QString &)), this,
          SLOT(handleTextChanged_depositAmount(const QString &)));
  connect(ui->lineEdit_deposit_placementPeriod,
          SIGNAL(textChanged(const QString &)), this,
          SLOT(handleTextChanged_depositPlacementPeriod(const QString &)));

  connect(ui->lineEdit_deposit_placementPeriod, SIGNAL(returnPressed()), this,
          SLOT(handleDepositCalculateButtonClick()));
  connect(ui->lineEdit_deposit_depositAmount, SIGNAL(returnPressed()), this,
          SLOT(handleDepositCalculateButtonClick()));

  connect(ui->pushButton_deposit_clear, SIGNAL(clicked()), this,
          SLOT(handleDepositClearButtonClick()));

  connect(ui->pushButton_deposit_taxes_logs, SIGNAL(clicked()), this,
          SLOT(handleDepositTaxesLogsButtonClick()));
  connect(ui->pushButton_deposit_payments_logs, SIGNAL(clicked()), this,
          SLOT(handleDepositLogsButtonClick()));
...
void MainWindow::connectCalculatorButtons() {
  ...
  for (int i = 0; i < buttonsCount; i++) {
    ...
    connect(expressionCalculatorButtons[i], SIGNAL(clicked()), this,
            SLOT(handleInsertButtonClick()));
  }
};
...
void MainWindow::connectTabButtons() {
  ...
  for (int i = 0; i < buttonsCount; ++i)
    connect(tabButtons[i], SIGNAL(clicked()), this, SLOT(handleTabClick()));
};
...
void MainWindow::connectCalculatorFunctionsButtons() {
  ...
  for (int i = 0; i < buttonsCount; i++) {
    ...
    connect(expressionCalculatorFunctionButtons[i], SIGNAL(clicked()), this,
            SLOT(handleInsertButtonClick()));
  }
};
...
void MainWindow::handleTabClick() {
  ...
  if (clickedButton == ui->pushButton_calculator) ui->lineEdit->setFocus();
...
void MainWindow::handleTextChanged_lineEdit(const QString &inputString) {
  handleTextChanged(ui->lineEdit, &Settings::errorCodeExpression);
}

void MainWindow::handleTextChanged_variable(const QString &inputString) {
  handleTextChanged(ui->lineEdit_X, &Settings::errorCodeVariable);
}

void MainWindow::handleTextChanged_domainEdit(const QString &inputString) {
  handleTextChanged(ui->lineEdit_domain, &Settings::errorCodeDomain);
}

void MainWindow::handleTextChanged_codomainEdit(const QString &inputString) {
  handleTextChanged(ui->lineEdit_codomain, &Settings::errorCodeCodomain);
}

void MainWindow::handleTextChanged_graphExpression(const QString &inputString) {
  handleTextChanged(ui->lineEdit_graph_expression, &Settings::errorCodeGraph);
}

void MainWindow::handleTextChanged_creditAmount(const QString &inputString) {
  handleTextChanged(ui->lineEdit_credit_creditAmount,
                    &Settings::errorCodeCreditAmount);
}

void MainWindow::handleTextChanged_creditDeadline(const QString &inputString) {
  handleTextChanged(ui->lineEdit_credit_creditDeadline,
                    &Settings::errorCodeCreditDeadline);
}

void MainWindow::handleTextChanged_interestValue(const QString &inputString) {
  handleTextChanged(ui->lineEdit_credit_interestValue,
                    &Settings::errorCodeCreditInterestValue);
}
void MainWindow::handleTextChanged_depositAmount(const QString &inputString) {
...
int MainWindow::getVariableLabelValue(double *pVariableValue) {
  ...
  if (!errorCode) *pVariableValue = result;
...
void MainWindow::handleButtonCreditCalculate() {
  ...
  if (!errorCode) outputCreditResult(&creditResult);
}
...
void handleSetCapitalisationStatus(DepositParameters *depositParams,
  ...
  if (checkBoxStatus) capitalisationStatus = CapitalisationStatus_on;
...
void MainWindow::handleDepositCalculateButtonClick() {
  ...
  errorCode |= refillsWidget->setRefills(&depositConfig.params);
  errorCode |=
      partialWithdrawalsWidget->setPartialWithdrawals(&depositConfig.params);
