...
int isFirstTaxForYear(DepositParameters *params, int year) {
  //    printf("params->depositDates.depositStartDate.tm_year: %d, year: %d\n",
  //           params->depositDates.depositStartDate.tm_year, year);
...
double calculateTaxesFromAnnualInterestValue(DepositConfig *depositConfig,
  ...
  if (taxAmount > 0) result->allYearsTaxes += taxAmount;
