...
int handleCalculationOperators(Stack *calculationStack, Data token) {
    ...
    if (!errorCode) {
        ...
        if (operand2.dataType == DataType_none)
            ...
            errorCode = performOperatorCalculations(token,
                                                    operand1,
                                                    operand2,
                                                    &result);
        }
    ...
    handlePushingResultValueInCalculationStack(calculationStack,
                                               result,
                                               errorCode);
...
int performFunctionCalculations(Data token, Data operand, double *pResult) {
    ...
    else if (token.functionType & (FunctionType_tangent | FunctionType_tangent_rad))
...
int handleFinalResultAssigning(Stack *calculationStack, double *pResult) {
    ...
    if (stack_getLength(calculationStack) != 1) {
        ...
        *pResult = finalResult.value;
        if (isnan(*pResult)) errorCode = ErrorCode_resultCalculatedValueIsNan;
    }

    return errorCode;
}

int calculateExpression(Stack *postfixNotation, double xVariable, double *pResult) {
    int errorCode = ErrorCode_noError;
    ...
    *pResult = 0;
    ...
    while (!stack_isEmpty(postfixNotation) && !errorCode) {
        ...
        if (token.dataType == DataType_none) {
            ...
            errorCode = handleCalculationsOperatorsAndFunctions(calculationStack,
                                                                token);
        }
    }
