...
int isOperator(char c) {
    return (c == S21_SYMBOL_PLUS || c == S21_SYMBOL_DASH ||
            ...
            c == S21_SYMBOL_OPEN_PARENTHESIS || c == S21_SYMBOL_CLOSE_PARENTHESIS);
}
...
int handleTokenizeCurrentToken(Stack *stack,
                               char *token,
                               int *pTokenIndex,
...
void writeTokenFromExpression(const char *expression,
                              char *token,
                              int *pSymbolIndex,
                              int *pTokenIndex) {
...
void handleUnaryOperatorsAfterOpenBracket(Stack *infixNotation) {
    ...
    binaryOperator.operatorType = fromBinaryToUnaryCode(binaryOperator.operatorType);
...
void tokenizeImplicitOperators(Stack *infixNotation, TokenStatus *pTokenStatus) {
...
int fromExpressionToInfixNotation(const char *expression, Stack *infixNotation) {
    ...
    for (int symbolIndex = 0; symbolIndex < expressionLength && !errorCode;
         symbolIndex++) {
        if (isWhitespace(expression[symbolIndex])) continue;
        if (isOperator(expression[symbolIndex])) {
            errorCode = handleTokenizeCurrentToken(infixNotation,
                                                   tokenString,
                                                   &tokenStringIndex,
                                                   &tokenStatus);

            handleTokenizeCurrentOperator(infixNotation,
                                          expression,
                                          symbolIndex,
                                          &tokenStatus);

        } else if (isalpha(expression[symbolIndex])) {
            writeTokenFromExpression(expression,
                                     tokenString,
                                     &symbolIndex,
                                     &tokenStringIndex);
            ...
            if ((data.functionType != FunctionType_none ||
                 data.dataType & (DataType_variable | DataType_value | DataType_operatorType)) && !errorCode) {
    ...
    if (!errorCode) {
        errorCode = handleTokenizeCurrentToken(infixNotation, tokenString, &tokenStringIndex, &tokenStatus);
    }
