...
int handleBrackets(int *pFlag, Data token, Stack *operatorStack, Stack *postfixStack) {
    int errorCode = ErrorCode_noError;
...
int isBracket(Data token) {
    return (token.dataType == DataType_bracketType);
}
...
int isFunction(Data token) {
    return (token.dataType == DataType_functionType);
}

int isAdditionOrSubtraction(Data token) {
    return (token.operatorType & (OperatorType_addition | OperatorType_subtraction)) > 0;
}

int isMultiplicationDivisionOrModulus(Data token) {
    return (token.operatorType & (OperatorType_multiplication | OperatorType_division |
                                  OperatorType_modulus)) > 0;
}
...
int isUnaryOperator(Data token) {
    return (token.operatorType & (OperatorType_unaryPlus | OperatorType_unaryMinus)) > 0;
}
